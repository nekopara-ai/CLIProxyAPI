...
func TestUsageReporterSetTranslatedRequestMetadataSeparatesServiceTiers(t *testing.T) {
	ctx := usage.WithServiceTier(context.Background(), "auto")
	reporter := NewUsageReporter(ctx, "openai", "gpt-5.4", &cliproxyauth.Auth{
	...
	record := reporter.buildRecord(usage.Detail{TotalTokens: 3, ResponseServiceTier: "default"}, false)
	if record.BaseURL != "https://custom.example/v1" {
		t.Fatalf("base URL = %q, want https://custom.example/v1", record.BaseURL)
	}
	if record.ServiceTier != "auto" {
...
func TestUsageReporterPropagatesSessionHierarchy(t *testing.T) {
	...
	reporter.SetTranslatedRequestMetadata([]byte(`{"service_tier":"priority"}`), "claude")
	record := reporter.buildRecord(usage.Detail{TotalTokens: 100, ResponseServiceTier: "default"}, false, usage.Failure{})
	...
	if record.EffectiveServiceTier != "priority" || record.ResponseServiceTier != "default" {
		t.Fatalf("hierarchy record lost service tiers: effective=%q response=%q", record.EffectiveServiceTier, record.ResponseServiceTier)
	}
