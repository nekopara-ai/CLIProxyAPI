...
func TestPutConfigYAMLReloadsRuntimeWithoutFileWatcher(t *testing.T) {
	...
	if reloaded.Port != 18002 || reloaded.Fingerprint.CooldownSeconds == nil || *reloaded.Fingerprint.CooldownSeconds != 240 || reloaded.Fingerprint.IntervalSeconds == nil || *reloaded.Fingerprint.IntervalSeconds != 600 {
		t.Fatalf("runtime did not receive the complete saved configuration: %+v", reloaded.Fingerprint)
	}
}
