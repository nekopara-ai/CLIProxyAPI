...
const (
	...
	cooldownFailureScopeRefreshUnauthorized = "refresh_unauthorized"
)
...
func hasUnauthorizedAuthFailure(auth *Auth) bool {
	...
	if auth.Unavailable && auth.Status == StatusError && auth.NextRefreshAfter.IsZero() &&
		...
		// Model aggregates can retain a 401 diagnostic after recovery. Only a
		// rejected refresh overrides a finite deadline; legacy terminal states
		// without any recovery deadline retain their existing classification.
		return auth.lastFailureScope == cooldownFailureScopeRefreshUnauthorized ||
			(auth.NextRetryAfter.IsZero() && auth.Quota.NextRecoverAt.IsZero() && auth.ForcedCooldownUntil.IsZero())
	}
	return false
}
